...
extension APIClient {
    ...
    // MARK: My Franchise (card collection)

    func franchise(token: String) async throws -> FranchiseSummary {
        let data = try await perform("GET", apiURL("franchise"), token: token, body: Optional<Int>.none)
        return try Self.decode(FranchiseSummary.self, from: data)
    }

    func franchiseCollection(token: String) async throws -> [PlayerCard] {
        let url = apiURL("franchise").appendingPathComponent("collection")
        let data = try await perform("GET", url, token: token, body: Optional<Int>.none)
        return try Self.decode(CollectionResponse.self, from: data).cards
    }

    func franchiseShop(token: String) async throws -> ShopResponse {
        let url = apiURL("franchise").appendingPathComponent("shop")
        let data = try await perform("GET", url, token: token, body: Optional<Int>.none)
        return try Self.decode(ShopResponse.self, from: data)
    }

    func franchiseBuy(playerId: String, token: String) async throws -> Int {
        let url = apiURL("franchise").appendingPathComponent("shop").appendingPathComponent("buy")
        struct Body: Encodable { let playerId: String }
        let data = try await perform("POST", url, token: token, body: Body(playerId: playerId))
        return try Self.decode(BuyResponse.self, from: data).coins
    }

    func franchiseLineup(token: String) async throws -> LineupResponse {
        let url = apiURL("franchise").appendingPathComponent("lineup")
        let data = try await perform("GET", url, token: token, body: Optional<Int>.none)
        return try Self.decode(LineupResponse.self, from: data)
    }

    func franchiseSetLineup(slot: String, cardId: String?, token: String) async throws {
        let url = apiURL("franchise").appendingPathComponent("lineup")
        struct Body: Encodable { let slot: String; let cardId: String? }
